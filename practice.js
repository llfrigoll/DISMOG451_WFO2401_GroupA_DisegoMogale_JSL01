...
function validateSyntax(){
    ...
    function yearValidation(input) {
        ...
        const input_year = input.substring(4, 8);
            ...
            let regex =  /^[0-9]+$/;
                ...
                if ((input_year < 1900) || (input_year > current_year)) {
                validYear = false;
                } 
    ...
    const validPet= valPet(inputElement)
    const validYear= yearValidation(inputElement)
    const validPetNamed = valPetNamed(inputElement) // !in the first pictures i was referencing the valid syntax function, not the valPetNamed one
